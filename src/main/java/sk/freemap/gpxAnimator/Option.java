...
public enum Option {
	...
	GUI("gui", "show GUI"), INPUT("input", "input GPX filename"),
	OUTPUT("output",
			"filename for generated video or filename template for saved image frames where %06d will be replaced by frame sequence number"),
	LABEL("label", "text displayed next to marker"), COLOR("color", "track color in #RRGGBB representation"),
	MARGIN("margin", "margin in pixels"), TIME_OFFSET("time-offset", "time offset for track in milliseconds"),
	FORCED_POINT_TIME_INTERVAL("forced-point-time-interval",
			"interval between adjanced GPS points in milliseconds - useful for GPX files with missing point time information; "
	...
	FPS("fps", "frames per second"), MARKER_SIZE("marker-size", "marker size in pixels"),
	...
	WIDTH("width",
			"video width in pixels; if not specified but zoom is specified, then computed from GPX bounding box and margin, otherwise 800"),
	...
	ATTRIBUTION("attribution",
			"map attribution text; %MAP_ATTRIBUTION% placeholder is replaced by attribution of selected pre-defined map (only from GUI)"),
	...
	TILE_CACHE_AGE_LIMIT("tile-cache-timeout", "time a cached map tile is valid"), HELP("help", "this help");
