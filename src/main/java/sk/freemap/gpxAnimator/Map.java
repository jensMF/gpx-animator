...
public class Map {
	...
	public static void drawMap(final BufferedImage bi, final String tmsUrlTemplate, final float backgroundMapVisibility,
			final int zoom, final double minX, final double maxX, final double minY, final double maxY,
			final RenderingContext rc, String tileCachePath, Long tileCacheTimeLimit) throws UserException {
		...
		for (int x = tileX; x <= maxXtile; x++) {
			for (int y = tileY; y >= maxYtile; y--) {
				...
				String url = tmsUrlTemplate.replace("{zoom}", Integer.toString(zoom))
				...
				final BufferedImage tile1 = new BufferedImage(tile.getWidth(), tile.getHeight(),
						BufferedImage.TYPE_INT_RGB);
				tile1.getGraphics().drawImage(tile, 0, 0, null);

				ga.drawImage(tile1, new RescaleOp(backgroundMapVisibility, (1f - backgroundMapVisibility) * 255f, null),
						256 * (x - tileX) + offsetX, bi.getHeight() - (256 * (tileY - y) + offsetY));
			}
		}
	}
